...
{ AS_blockList list = (AS_blockList)checked_malloc (sizeof *list);
  list->head=head;
  list->tail=tail;
  return list;
}
...
{ AS_block b = (AS_block)checked_malloc (sizeof *b);
  AS_instrList last;
  b->instrs = instrs;
  if (instrs->head->kind == AS_instr_::I_LABEL) b->label = instrs->head->u.LABEL.label;
  else assert(0); //has to be a label!
  last=instrs; 
  while (last->tail) last=last->tail;
  if (last->head->kind == AS_instr_::I_OPER) {
...
static bool isCjump(AS_instr s){
  if(s->kind == AS_instr_::I_OPER && s->u.OPER.assem[0] == 'b'){
    if(s->u.OPER.jumps->labels && s->u.OPER.jumps->labels->tail && !s->u.OPER.jumps->labels->tail->tail){
      return true;
    }
  }
  return false;
...
static AS_instr notRel(AS_instr s){
  char ins[100];
  if(s->u.OPER.assem[1] == 'n'
  && s->u.OPER.assem[2] == 'e'){
    sprintf(ins, "beq .`j0");
  }
  if(s->u.OPER.assem[1] == 'e'
  && s->u.OPER.assem[2] == 'q'){
    sprintf(ins, "bne .`j0");
  }
  if(s->u.OPER.assem[1] == 'l'
  && s->u.OPER.assem[2] == 't'){
    sprintf(ins, "bge .`j0");
  }
  if(s->u.OPER.assem[1] == 'g'
  && s->u.OPER.assem[2] == 'e'){
    sprintf(ins, "blt .`j0");
  }
  if(s->u.OPER.assem[1] == 'g'
  ...
  // reverse relop and reverse label
  return AS_Oper(String(ins), NULL, NULL, AS_Targets(Temp_LabelList(s->u.OPER.jumps->labels->tail->head, Temp_LabelList(s->u.OPER.jumps->labels->head, NULL))));
}
...
static void trace(AS_instrList list, AS_instrList epilog, Bool optimize) //this list is a basic block
{
  ...
  */ 
  if (s->kind != AS_instr_::I_OPER ) assert(-1); // can't be a label of move!
  ...
  if(target){
    ...
    // can't find the jump-to block! fail
    assert(0);
    last->tail->tail = getNext(epilog, optimize); 
  }
}
...
static bool isJump(AS_instr s){
  if(!s) return false;
  if(s->kind == AS_instr_::I_OPER && s->u.OPER.assem[0] == 'b'){
    if(s->u.OPER.jumps && s->u.OPER.jumps->labels && !s->u.OPER.jumps->labels->tail){
      return true;
    }
  }
  return false;
...
void eliminateBnext(AS_instrList arm_ail){
  if(!arm_ail) return;
  assert(!isJump(arm_ail->head)); // so i do not delete arm_ail->head
  AS_instrList cur = NULL, pre = NULL;
  for(cur=arm_ail; cur; pre=cur, cur=cur->tail){
    if(cur && isJump(cur->head) && cur->tail && cur->tail->head->kind == AS_instr_::I_LABEL){
      if(cur->head->u.OPER.jumps->labels->head == cur->tail->head->u.LABEL.label){
        // cur is jump
        // delete cur jump
        // printf("delete b label %s\n", Temp_labelstring(cur->tail->head->u.LABEL.label));
        arm_ail = AS_instrList_replace_opt(arm_ail, &pre, &cur, NULL);
      }
    }
  }
}
