...
TempSet FG_LLVM::FG_def(G_node n) {
	return UD_lookup(n)->defs;
}

TempSet FG_LLVM::FG_use(G_node n) {
	return UD_lookup(n)->uses;
}

bool FG_LLVM::FG_isMove(G_node n) {
    return UD_lookup(n)->isMove;
}

void FG_LLVM::FG_Showinfo(FILE *out, AS_instr instr, Temp_map map) {
	char* cs;
	char* lb;
    char r[200]; /* result */
	switch(instr->kind) {
...
#define IT_MOVE 2
#define IT_RETURN 3
G_graph FG_LLVM::FG_AssemFlowGraph(LLVM_IR::T_irList_ *il) {
	UD_init();

	//(I) Iterate over the entire instruction list
	LLVM_IR::T_ir_ *ir = NULL;
	G_node prev = NULL;
	G_node curr = NULL;
	...
	for(; il!=NULL; il=il->tail){
		...
		if(ir!=NULL){
			...
			case AS_instr_::I_OPER:
				...
				if(ir->i->u.OPER.jumps!=NULL &&
					ir->i->u.OPER.jumps->labels!=NULL){
					type = IT_JUMP;
				...
				if(ir->s && ir->s->kind == LLVM_IR::llvm_T_stm_::T_RETURN){
					type = IT_RETURN;
				}
				if(ir->s && ir->s->kind == LLVM_IR::llvm_T_stm_::T_PHI){
					type = IT_COMMON;
				}
				defs = makeTempSet(ir->i->u.OPER.dst);
				uses = makeTempSet(ir->i->u.OPER.src);
				break;
			case AS_instr_::I_LABEL:
				//2.2) label should be also saved in the label-node list for (II)
				defs = makeTempSet(nullptr);
				uses = makeTempSet(nullptr);
				LT_enter(ir->i->u.LABEL.label, curr);
				break;
			case AS_instr_::I_MOVE:
				//2.3) it's a move instruction
				type = IT_MOVE;
				defs = makeTempSet(ir->i->u.MOVE.dst);
				uses = makeTempSet(ir->i->u.MOVE.src);
				break;
			}
	...
	for(; jumpListHead!=NULL; jumpListHead=jumpListHead->tail){
		...
		labels = ((LLVM_IR::T_ir_ *)G_nodeInfo(curr))->i->u.OPER.jumps->labels;//no need to check its nullity again
		Temp_label label;
		G_node dest;
