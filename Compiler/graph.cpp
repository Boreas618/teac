...
static G_nodeList deleteNode(G_node a, G_nodeList l) {
  assert(a && l);
  if (a==l->head) return l->tail;
  else return G_NodeList(l->head, deleteNode(a, l->tail));
}

void G_rmEdge(G_node from, G_node to) {
  assert(from && to);
  to->preds=deleteNode(from,to->preds);
  from->succs=deleteNode(to,from->succs);
}

 /**
  * Print a human-readable dump for debugging.
  */
void G_show(FILE *out, G_nodeList p, void showInfo(void *, FILE *)) {
  for (; p!=NULL; p=p->tail) {
    G_node n = p->head;
    G_nodeList q;
    assert(n);
...
{
  if(t.find(node) != t.end()){
    return t.find(node)->second;
  }else{
    return NULL;
  }
}

G_node GetNode(int *mykey){
    return container_of(mykey, G_node_ , mykey);
}

int *GetMykey(G_node n){
    assert(n);
    return &(n->mykey);
}
