...
pub fn allocate(instructions: &[Inst]) -> AllocationResult {
    ...
    let liveness = LivenessAnalysis::compute(instructions, &cfg);
...
impl ControlFlowGraph {
    fn build(instructions: &[Inst]) -> Self {
        let n = instructions.len();

        let label_map: HashMap<&str, usize> = instructions
            .iter()
            .enumerate()
            .filter_map(|(i, inst)| {
        ...
        let successors: Vec<Vec<usize>> = instructions
            .iter()
            .enumerate()
            .map(|(i, inst)| Self::compute_successors(inst, i, n, &label_map))
            .collect();
