...
void load_register(list<AS_stm *> &as_list)
{
    ...
    for (auto it = allocateRegs.begin(); it != allocateRegs.end(); ++it)
    {
        ...
        {
            int second = *it;
            rbegin = as_list.insert(rbegin, AS_Ldp(new AS_reg(AS_type::Xn, first), new AS_reg(AS_type::Xn, second), sp, 2 * 8));
        }
        else
        {
            // 如果`set`中的元素个数是奇数，最后一个元素将单独处理
            rbegin = as_list.insert(rbegin, AS_Ldr(new AS_reg(AS_type::Xn, first), sp, 8));
            break;
        }
    }
    rbegin = as_list.insert(rbegin, AS_Ldp(new AS_reg(AS_type::Xn, FPR), new AS_reg(AS_type::Xn, LNR), sp, 2 * 8));
}
