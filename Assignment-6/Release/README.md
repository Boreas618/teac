...
`LAB6` 为两个主体部分：1️⃣ **指令选择**和 2️⃣ **寄存器分配**，整体工作量为500 loc。
...
```
在帮助文档中，我们给出函数 `ASM::printAS_decl` 的实现范例，课程参与者可依据范例实现其他内容，工作量在 `100 loc` 。
```cpp
// define i32 @main( )
// .global main
...
`llvm2asm.cpp` 需要实现完整的翻译任务，对于每个语句的翻译流程，我们在此处给出提示，总工作量在 `400 loc` 左右。
